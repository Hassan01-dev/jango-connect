...
export const FriendRequest = () => {
  ...
  const [approveFriendRequest, { isLoading: isApproving }] = useApproveFriendRequestMutation();
  ...
  const handleApprove = async (friendRequestId: string) => {
    try {
      await approveFriendRequest({ friendRequestId }).unwrap();
    } catch (err) {
      console.error('Failed to approve friend request:', err);
    }
  };

  const handleReject = async (friendRequestId: string) => {
    try {
      await rejectFriendRequest({ friendRequestId }).unwrap();
    } catch (err) {
